...
const tabs: { id: Tab; icon: typeof ImageIcon; label: string; hint: string }[] = [
  { id: 'image', icon: ImageIcon, label: 'Image', hint: 'Photo of the pest or damage' },
  { id: 'video', icon: Video, label: 'Video', hint: 'Footage of the infestation' },
            ...
            <div className="rounded-xl border border-border bg-background/60 p-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h3 className="font-heading text-xl font-bold">
                  Prediction: {result.pest || 'Unknown'}
                </h3>
                {result.confidence && (
              ...
              <dl className="mt-4 space-y-3 text-sm">
                {result.symptoms && <ResultRow label="Damage symptoms" value={result.symptoms} />}
                {result.control && <ResultRow label="Control methods" value={result.control} />}
              </dl>
            </div>
            <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-700">
              <strong>Heads up:</strong> this is an AI-assisted estimate. Always follow
              local regulations and safety guidelines when applying pest control products.
            </div>
          </div>
        </div>
      )}
